                  ...
                  match Perso_link.get_parents_link base_prefix ip with
                    Some family ->
                      let (ifath, imoth, base_prefix) =
                        Adef.iper_of_int
                          (Int32.to_int family.MLink.Family.ifath),
                        Adef.iper_of_int
                   ...
                   Some family ->
                     let (ifath, imoth, ifam) =
                       ...
                       Adef.iper_of_int
                         (Int32.to_int family.MLink.Family.imoth),
                       Adef.ifam_of_int
                         (Int32.to_int family.MLink.Family.ifam)
                     in
                     begin match
...
let string_of_source2 conf base s =
  let src_with_link d_src src opt =
    if conf.cancel_links then src
    else
...
and eval_simple_str_var conf base env (_, p_auth) =
  ...
  | "source_type2" ->
      begin match get_env "src_typ2" env with
        Vstring s -> s
      | _ -> raise Not_found
      end
  | "surname_alias" ->
            ...
            let (f, c, a) =
              let rec loop l =
                ...
                | fam_link :: l ->
                    let fam_link_ifam =
                      Adef.ifam_of_int
                        (Int32.to_int fam_link.MLink.Family.ifam)
                    in
                    if fam_link_ifam = ifam then
                      ...
                      let (ifath, imoth) =
                        ...
                        Adef.iper_of_int
                          (Int32.to_int fam_link.MLink.Family.imoth)
                      in
                      let cpl =
                        let ip = get_key_index p in
                        if ip <> ifath && ip <> imoth then
                          ...
                          with
                            Some p ->
                              let ip =
                                Adef.iper_of_int
                                  (Int32.to_int p.MLink.Person.ip)
                              in
                              ifath, imoth,
...
and eval_str_person_field conf base env (p, p_auth as ep) =
  ...
  | "source2" ->
      begin match get_env "src2" env with
        Vstring s ->
          let param =
              ...
              (fun fam_link ->
                 ...
                 List.iter
                   (fun c_link ->
                      ...
                      let ip_c =
                        Adef.iper_of_int
                          (Int32.to_int c_link.MLink.Person_link.ip)
                      in
